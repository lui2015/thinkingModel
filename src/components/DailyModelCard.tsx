...
export default function DailyModelCard({ initialModel, models }: DailyModelCardProps) {
  ...
  // idle = 稳定；out = 翻出；in = 翻入
  const [phase, setPhase] = useState<"idle" | "out" | "in">("idle");
  const [burstKey, setBurstKey] = useState(0); // 每次点击触发一组新的光斑/粒子
  ...
  const contentFlipClass =
    phase === "out" ? "daily-flip-out" : phase === "in" ? "daily-flip-in" : "";
  const isAnimating = phase !== "idle";

  return (
    <Link href={`/models/${model.slug}`} className="block group">
      <div
        className={`relative overflow-hidden rounded-3xl p-6 sm:p-8 text-white shadow-xl transition-[transform,box-shadow,background] duration-500 ease-out hover:-translate-y-0.5 ${
          isAnimating ? "daily-bump" : ""
        }`}
        style={{
          background: `linear-gradient(135deg, ${gradientFrom} 0%, ${gradientTo} 100%)`,
          boxShadow: isAnimating
            ? `0 30px 60px -20px ${catColor}aa, 0 0 0 1px ${catColor}33 inset`
            : `0 20px 40px -18px ${catColor}66`,
        }}
      >
        {/* 背景装饰 */}
        <div
        ...
        <div
          ...
          style={{ background: "radial-gradient(circle, #fff 0%, transparent 70%)" }}
        />
        <div className="pointer-events-none absolute right-6 top-4 text-[120px] leading-none opacity-[0.08] select-none transition-opacity duration-500">
          {model.icon}
        </div>

        {/* 光斑扫过（每次点击重建） */}
        {burstKey > 0 && (
          <div key={`shimmer-${burstKey}`} className="daily-shimmer" />
        )}
          ...
          <div className="relative">
            ...
            {burstKey > 0 && (
              ...
              >
                {/* 光环 */}
                <span
                  className="daily-ring absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full"
                  style={{ boxShadow: `0 0 0 2px #fff, 0 0 18px 4px ${catColor}` }}
                />
                {particles.map((p, i) => (
                  <span
                    key={i}
                    className="daily-particle absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full"
                    style={
                      {
                        width: p.size,
                        height: p.size,
                        background: i % 2 === 0 ? "#fff" : catColor,
                        boxShadow: `0 0 8px ${i % 2 === 0 ? "#fff" : catColor}`,
            ...
            <button
              ...
              aria-label="换一个"
              title="换一个"
              className="relative z-20 inline-flex items-center gap-1.5 rounded-full bg-white/15 hover:bg-white/25 active:scale-95 backdrop-blur-sm ring-1 ring-white/20 px-3 py-1.5 text-xs font-medium transition-all disabled:cursor-not-allowed"
            >
              <svg
                className={`w-3.5 h-3.5 transition-transform ${
                  isAnimating ? "animate-[spin_0.8s_cubic-bezier(.22,1,.36,1)]" : ""
                }`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
        ...
        <div className="relative [perspective:1200px]">
          <div
            className={`[transform-style:preserve-3d] ${contentFlipClass}`}
